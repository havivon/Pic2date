 ...
 *
 * If the trained data is not bundled, [recognize] throws
 * [OcrUnavailableException] so callers can fall back to another engine.
 */
class TesseractOcrEngine(
    ...
    private fun ensureApi(): TessBaseAPI {
        ...
        val ok = instance.init(dataDir.absolutePath, languages)
        if (!ok) {
            instance.recycle()
